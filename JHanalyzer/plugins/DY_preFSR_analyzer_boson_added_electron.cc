...
//DY_preFSR_analyzer_boson_added_electron
//
//LHEEventProduct
//externalLHEProducer
//LHERunInfoProduct
...
class DY_preFSR_analyzer_boson_added_electron : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   public:
      explicit DY_preFSR_analyzer_boson_added_electron(const edm::ParameterSet&);
      ~DY_preFSR_analyzer_boson_added_electron();
  ...
  //To validate our preFSR definition
  // TTree *status2223;//hardprocess boson OR lepton pair info (DY boson info)
  TTree *lheinfo;//lhe level info
  TTree *DYboson;


  ///weight from muR/muF variation//
  ...
  //weight from PDF systematic replica
  TTree *trupdfweight0;
  TTree *trupdfweight1;
  TTree *trupdfweight2;
  TTree *trupdfweight3;
  ...
  double DR1,DR2, isfsr;//dR(DY_l,photons) distribution
  ...
  double fsr_pl_px, fsr_pl_py, fsr_pl_pz,fsr_pl_ee; //FSR lepton + photons



  double fsr10_1_px, fsr10_1_py, fsr10_1_pz,fsr10_1_ee;
  double fsr10_2_px, fsr10_2_py, fsr10_2_pz,fsr10_2_ee;//For dressed lepton
  ...
  double fsr001_1_px, fsr001_1_py, fsr001_1_pz,fsr001_1_ee;
  double fsr001_2_px, fsr001_2_py, fsr001_2_pz,fsr001_2_ee;//For dressed lepton                                                                                   
  ...
  double boson_m,boson_pt;//, boson_phi,boson_eta;
  double NFSR,Zevent, gammaevent;
...
//
DY_preFSR_analyzer_boson_added_electron::DY_preFSR_analyzer_boson_added_electron(const edm::ParameterSet& iConfig)
...
DY_preFSR_analyzer_boson_added_electron::~DY_preFSR_analyzer_boson_added_electron()
{
...
void
DY_preFSR_analyzer_boson_added_electron::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
trupdfweight101, w101
  ...
  fsr10_1_px=0, fsr10_1_py=0, fsr10_1_pz=0,fsr10_1_ee=0;
  fsr10_2_px=0, fsr10_2_py=0, fsr10_2_pz=0,fsr10_2_ee=0;//For dressed lepton                                                                                   
  ...
  fsr001_1_px=0, fsr001_1_py=0, fsr001_1_pz=0,fsr001_1_ee=0;                                                        
  fsr001_2_px=0, fsr001_2_py=0, fsr001_2_pz=0,fsr001_2_ee=0;//For dressed lepton  
...
//////////////////////
   ...
   int leppid=11;//for Muon channel
   ...
   TLorentzVector v1lhe,v2lhe,vzglhe;//1st leptin/2nd lepton/ Zorgamma in LHE level 
   lhemass=0;
   ...
   vector<int> hardindex; // to save indices of  Z/gamma boson
   ...
   //int Z_i_status22=-99;
   int Z_i_hard = -99;//status 21-24
   int gamma_i_hard = -99;//status21-24
   //cout<<"set hardindex(default)"<<endl;
   for(int i = 0; i < gensize; ++ i) {///scan all gen particles
     const GenParticle & p = (*genParticles)[i];
     //     int hardprocess = p.isHardProcess();//don't use statusflag. only status code
     int id = p.pdgId();
     ...
     if(id==23 && status>20 && status<25) Z_i_hard=i; //if it is Z boson from hardprocess 
     if(id==22 && status>20 && status<25) gamma_i_hard=i;//if it is gamma boson from hardprocess
...
//     }//genparticle loop
  ...
  //  int hardindexsize_default=hardindex_default.size();
   ...
   //&& final prompt lepton
   //cout<<"Z_i_hard"<<Z_i_hard<<endl;
   // cout<<"gamma_i_hard"<<gamma_i_hard<<endl;
  ...
  //cout<<"set motherindex"<<endl;
   ...
   vector<int> gen_fromhardDYZ;
   vector<int> gen_fromhardDYgamma;
   ...
   for(int i = 0; i < gensize; ++ i) {
     ...
     int fromhardDY=0;
     int fromhardDYZ=0;
     int fromhardDYgamma=0;
     int mother = gen_motherindex[i];
     if(mother<0){
       gen_fromhardDY.push_back(fromhardDY);
       gen_fromhardDYZ.push_back(fromhardDYZ);
       gen_fromhardDYgamma.push_back(fromhardDYgamma);
       continue;
     } 
   ...
   //   cout<<"gen_fromhardDY.size="<<gen_fromhardDY.size()<<endl;
   //cout<<"gen_fromhardDYZ.size="<<gen_fromhardDYZ.size()<<endl;
   //cout<<"gen_fromhardDYgamma.size="<<gen_fromhardDYgamma.size()<<endl;
 
  ///let's find tau -> if its from hard DY -> veto
   //and find prompt lepton pair
   //cout<<"tau veto"<<endl;
   // cout<<"before tauveto"<<endl;
   for(int i = 0; i < gensize; ++ i) {
     const GenParticle & p = (*genParticles)[i];
     int id = p.pdgId();
	  //     int status = p.status();
     ...
     //     int prompt = p.statusFlags().isPrompt();//                                                                                                                                               
     int mother=gen_motherindex[i];
          if(fromhardDY)     cout<<"i="<<i<<" pid="<<id<<" status="<<status<<" mother="<<mother<<" px="<<px<<" py="<<py<<" pz="<<pz<<" ee="<<ee<<endl;
     */
   }   
   //   if(nhard==0){
   ...
   for(int i = 0; i < gensize; ++ i) {
       ...
       //double pz = p.pz();
       //double ee = p.energy();
       //   cout<<"i="<<i<<" id="<<id<<" status="<<status<<" fromhardDY="<<fromhardDY<<endl;
...
// }
   ...
   //vlep1_i.size//
   //let's determine prompt lepton pair if there are more than 1 lepton pair
   //cout<<"before 0 lep"<<endl;
   ...
   for(int i = 0; i < gensize; ++ i) {
     ...
     if(id==22){ nphoton+=1;
       ...
       if(dR1<1.0){
         fsr10_1_px=px;          fsr10_1_py=py;          fsr10_1_pz=pz;          fsr10_1_ee=ee;
         // fsr04_2_px=px;       fsr04_2_py=py;          fsr04_2_pz=pz;          fsr04_2_ee=ee; 
       }
       else if(dR2<1.0){
         //fsr04_1_px=px;          fsr04_1_py=py;          fsr04_1_pz=pz;          fsr04_1_ee=ee; 
         fsr10_2_px=px;          fsr10_2_py=py;          fsr10_2_pz=pz;          fsr10_2_ee=ee;
       }
	 ...
	 fsr03_1_px=px;          fsr03_1_py=py;          fsr03_1_pz=pz;          fsr03_1_ee=ee;
       ...
       if(dR1<0.01){
         fsr001_1_px=px;          fsr001_1_py=py;          fsr001_1_pz=pz;          fsr001_1_ee=ee;
       }
       else if(dR2<0.01){
         fsr001_2_px=px;          fsr001_2_py=py;          fsr001_2_pz=pz;          fsr001_2_ee=ee;
       }
     ...
     */
     ...
     //status==1, !lep1 !lep2
       ...
       // cout<<"vlep1_i.size()="<<vlep1_i.size()<<endl;
       //cout<<"vlep2_i.size()="<<vlep2_i.size()<<endl;
       // cout<<"fsr's dR1="<<dR1<<endl;
       //cout<<"fsr's dR2="<<dR2<<endl;
       NFSR+=1;
       if(id==22){
	 isfsr=1;
  ...
  else if(gamma_i_hard > 0 ){
     double px,py,pz,ee;
     px=     (*genParticles)[gamma_i_hard].px();
     py=     (*genParticles)[gamma_i_hard].py();
     pz=     (*genParticles)[gamma_i_hard].pz();
     ee=     (*genParticles)[gamma_i_hard].energy();
     TLorentzVector v;
     v.SetPxPyPzE(px,py,pz,ee);
     boson_m=v.M();
     // boson_phi=v.Phi();                                                                                                                     
   ...
   */
   ...
   //last check
   //reco@gen Z/gamma momentum//                                                                                                                   

 /*                                           

   
   TLorentzVector vbo;
   vbo.SetPxPyPzE(lep1_px+lep2_px+fsr_t_px,lep1_py+lep2_py+fsr_t_py,lep1_pz+lep2_pz+fsr_t_pz,lep1_ee+lep2_ee+fsr_t_ee);
 
   cout<<"===========event info============"<<endl;
   cout<<"lep1_i="<<lep1_i<<endl;
   cout<<"lep2_i="<<lep2_i<<endl;
   cout<<"Z_i_hard="<<Z_i_hard<<endl;
   cout<<"gamma_i_hard="<<gamma_i_hard<<endl;

   cout<<"lhemass="<<lhemass<<endl;
   cout<<"boson_m="<<boson_m<<endl;
   cout<<"recoZ/gamma M="<<vbo.M()<<endl;

   cout<<"lhept="<<lhept<<endl;
   cout<<"boson_pt="<<boson_pt<<endl;
   cout<<"recoZ/gamma Pt="<<vbo.Perp()<<endl;
   
   cout<<"lep1_px="<<lep1_px<<" lep1_py="<<lep1_py<<" lep1_pz="<<lep1_pz<<" lep1_ee="<<lep1_ee<<endl;
   cout<<"lep2_px="<<lep2_px<<" lep2_py="<<lep2_py<<" lep2_pz="<<lep2_pz<<" lep2_ee="<<lep2_ee<<endl;
   cout<<"isr_t_px="<<isr_t_px<<" isr_t_py="<<isr_t_py<<" isr_t_pz="<<isr_t_pz<<" isr_t_ee="<<isr_t_ee<<endl;
   cout<<"fsr_t_px="<<fsr_t_px<<" fsr_t_py="<<fsr_t_py<<" fsr_t_pz="<<fsr_t_pz<<" fsr_t_ee="<<fsr_t_ee<<endl;
   cout<<"fsr10_1_px="<<fsr10_1_px<<" fsr10_1_py="<<fsr10_1_py<<" fsr10_1_pz="<<fsr10_1_pz<<" fsr10_1_ee"<<fsr10_1_ee<<endl;
  ...
  for(int i = 0; i < gensize; ++ i) {
     ...
     int status = p.status();
     int fromhardDY=gen_fromhardDY[i];
     int fromhardDYZ=gen_fromhardDYZ[i];
     int fromhardDYgamma=gen_fromhardDYgamma[i];
     int mother =  gen_motherindex[i];
     double eta=p.eta();
     double phi=p.phi();

     if(fromhardDY) cout<<"i="<<i <<" id="<<id<<" status="<<status<<" mother="<<mother<<" fromhardDY"<<fromhardDY<<" fromhardDYZ="<<fromhardDYZ<<" fromhardDYgamma="<<fromhardDYgamma<<" eta="<<eta<<" phi="<<phi<<endl;
			   }
*/
   //
...
void 
DY_preFSR_analyzer_boson_added_electron::beginJob()
{
  cout<<"beginjob"<<endl;
  ...
  fsr001_1 = fs->make<TTree>("fsr001_1","fsr001_1");
  fsr001_2 = fs->make<TTree>("fsr001_2","fsr001_2");
  ...
  fsr10_1 = fs->make<TTree>("fsr10_1","fsr10_1");
  fsr10_2 = fs->make<TTree>("fsr10_2","fsr10_2");
  ...
  DYboson = fs->make<TTree>("DYboson","DYboson");
  ...
  fsr001_1->Branch("px",&fsr001_1_px,"px/D");
  fsr001_1->Branch("py",&fsr001_1_py,"py/D");
  fsr001_1->Branch("pz",&fsr001_1_pz,"pz/D");
  fsr001_1->Branch("ee",&fsr001_1_ee,"ee/D");

  fsr001_2->Branch("px",&fsr001_2_px,"px/D");
  fsr001_2->Branch("py",&fsr001_2_py,"py/D");
  fsr001_2->Branch("pz",&fsr001_2_pz,"pz/D");
  fsr001_2->Branch("ee",&fsr001_2_ee,"ee/D");

  cout<<"beginjob2-2"<<endl;
  ...
  fsr10_1->Branch("px",&fsr10_1_px,"px/D");
  fsr10_1->Branch("py",&fsr10_1_py,"py/D");
  fsr10_1->Branch("pz",&fsr10_1_pz,"pz/D");
  fsr10_1->Branch("ee",&fsr10_1_ee,"ee/D");

  fsr10_2->Branch("px",&fsr10_2_px,"px/D");
  fsr10_2->Branch("py",&fsr10_2_py,"py/D");
  fsr10_2->Branch("pz",&fsr10_2_pz,"pz/D");
  fsr10_2->Branch("ee",&fsr10_2_ee,"ee/D");
  cout<<"beginjob3"<<endl;
  ...
  DYboson->Branch("Zevent", &Zevent,"Zevent/D");
  DYboson->Branch("gammaevent", &gammaevent,"gammaevent/D");
  DYboson->Branch("boson_m",&boson_m,"boson_m/D");
  //DYboson->Branch("boson_eta",&boson_eta,"boson_eta/D");
  //DYboson->Branch("boson_phi",&boson_phi,"boson_phi/D");
  DYboson->Branch("boson_pt",&boson_pt,"boson_pt/D");
  ...
  trupdfsclweight0->Branch("w0",&trupdfweight[0],"w0/D");
  trupdfsclweight1->Branch("w1",&trupdfweight[1],"w1/D");
  trupdfsclweight2->Branch("w2",&trupdfweight[2],"w2/D");
  trupdfsclweight3->Branch("w3",&trupdfweight[3],"w3/D");
  trupdfsclweight4->Branch("w4",&trupdfweight[4],"w4/D");
  trupdfsclweight5->Branch("w5",&trupdfweight[5],"w5/D");
...
void 
DY_preFSR_analyzer_boson_added_electron::endJob() 
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
DY_preFSR_analyzer_boson_added_electron::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(DY_preFSR_analyzer_boson_added_electron);
